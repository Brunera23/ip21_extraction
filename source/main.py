...
for gbu,sites in servers.items():
    for site,hostnames in sites.items():
        for hostname in hostnames:
            ...
            print('1/5')
            tags_df = fetch_tag(hostname,crediantials["Username"],crediantials["Password"])
            print('2/5')
            with get_connection(hostname,crediantials["Username"],crediantials["Password"]) as conn:
                ...
                print('4/5')
                outname = f"data/{gbu}/{site}/TagExport-{hostname}-{datetime.now():%Y%m%d}.csv"
                print('5/5')
                final_df.to_csv(outname.replace('.csv','.txt'),index=False,encoding="utf-8")
